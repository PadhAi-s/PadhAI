...
export function AdminDashboard() {
  ...
  function parseCSV(text: string): SyllabusRow[] {
    ...
    lines.slice(1).forEach((line, index) => {
      ...
      if (category !== "school" && category !== "government_exam") {
        throw new Error(
      ...
      if (!row.subject?.trim()) {
        throw new Error(`Row ${rowNumber}: subject is required.`);
      }

      if (!row.chapter_name?.trim()) {
        throw new Error(`Row ${rowNumber}: chapter_name is required.`);
      }
      ...
      if (!Number.isFinite(parsedOrder)) {
        throw new Error(`Row ${rowNumber}: order_no must be a number.`);
      }
                  ...
                  <div className="mt-6">
                    ...
                    {csvRows.length > 100 && (
                      <p className="mt-2 text-xs text-slate-500">
                        Showing first 100 rows. All {csvRows.length}{" "}
